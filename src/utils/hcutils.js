...
function encrypt(string) {
  ...
  return md5(md5(string) + str) + str;
}

const number = ['int', 'short', 'byte', 'long', 'float', 'double', 'java.lang.Integer', 'java.lang.Byte', 'java.lang.Short', 'java.lang.Long', 'java.lang.Float', 'java.lang.Double'];
const string = ['char', 'java.lang.Character', 'java.lang.String', 'java.lang.CharSequence', 'java.lang.StringBuilder', 'java.lang.StringBuffer'];
const boolean = ['boolean', 'java.lang.Boolean'];
const none = ['void', 'java.lang.Void'];

function typeHttp(type) {
  if (type.startsWith('[L')) {
    type = 'array[' + typeHttp(type.substring(2, type.length - 1)) + ']';
  } else if (type === '[I' || type === '[S' || type === '[J' || type === '[B' || type === '[F' || type === '[D') {
    type = 'array[number]';
  } else if (type === '[Z') {
    type = 'array[boolean]';
  } else if (type === '[C') {
...
function paramHttp(name, type) {
  if (name === '_HTTP_REQUEST_CLIENT_IP_ADDRESS_') {
    name = '@ip(系统自动获取)';
  }
  type = typeHttp(type);
  let demo = null;
  return {'name': name, 'type': type, 'demo': demo}
}

function paramRpc(name, type) {
  if (name === '_HTTP_REQUEST_CLIENT_IP_ADDRESS_') {
    name = '@ip(系统自动获取)';
  }
  if (type.startsWith('[L')) {
...
module.exports.paramHttp = paramHttp;
module.exports.typeHttp = typeHttp;
module.exports.paramRpc = paramRpc;
